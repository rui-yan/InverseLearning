% ADMM0
function [x,y,xnorm] = ADMM_div(x0,y0,beta,A, max_iter)
% @brief: ADMM to solve the zero objective case
...
% @returns: x norm during the optmization process
    ...
    for iter = 1:max_iter
        ...
        x(1) = -beta*A(:,1)'*A(:,1)\(A(:,1)'*b);
        b = beta * (x(1)*A(:,1) + x(3)*A(:,3))-y;
        x(2) = -beta*A(:,2)'*A(:,2)\(A(:,2)'*b);
        b = beta * (x(2)*A(:,2) + x(1)*A(:,1))-y;
        x(3) = -beta*A(:,3)'*A(:,3)\(A(:,3)'*b);
        y = y - beta*(A*x);
        xnorm(iter) =norm(x);
    end
end