% ADMM0
function [x,y,xnorm] = ADMM_divrand(x0,y0,beta,A, max_iter)
% @brief: ADMM to solve the zero objective case, random order
...
% @returns: x norm during the optmization process
    ...
    for iter = 1:max_iter
        ...
        x(p(1)) = -beta*A(:,p(1))'*A(:,p(1))\(A(:,p(1))'*b);
        b = beta * (x(p(1))*A(:,p(1)) + x(p(3))*A(:,p(3)))-y;
        x(p(2)) = -beta*A(:,p(2))'*A(:,p(2))\(A(:,p(2))'*b);
        b = beta * (x(p(2))*A(:,p(2)) + x(p(1))*A(:,p(1)))-y;
        x(p(3)) = -beta*A(:,p(3))'*A(:,p(3))\(A(:,p(3))'*b);
        y = y - beta*(A*x);
        xnorm(iter) =norm(x);
    end
end