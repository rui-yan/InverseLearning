function [x,y1,y2,x_hist] = block_update_pperm(x0,y10,y20,Q,A,b,beta,max_iter)
% @brief: ADMM to find minimum solution to 1/2xTQx using 5-block update,
%           update is randomly permuted
...
% @return: x_hist, x during iteration
    ...
    for k = 1:max_iter
        ...
        for i = 1:5
            ...
            x(p(i),:) = (diag(D_block(p(i),:)) + beta * A_block{p(i),1}' * A_block{p(i),1}...
                + beta * V_block{p(i),1}' * V_block{p(i),1})...
                \(A_block{p(i),1}' * y1 + V_block{p(i),1}' * y2 - beta * A_block{p(i),1}' * (r-b)...
                - beta * V_block{p(i),1}' * (rv-s));
        end
